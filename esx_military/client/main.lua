...
function OpenCloakroomMenu()
	...
	if grade == 'recruit' then
		table.insert(elements, {label = _U('military_wear'), value = 'recruit_wear'})
	elseif grade == 'officer' then
		table.insert(elements, {label = _U('military_wear'), value = 'officer_wear'})
	elseif grade == 'sergeant' then
		table.insert(elements, {label = _U('military_wear'), value = 'sergeant_wear'})
	elseif grade == 'intendent' then
		table.insert(elements, {label = _U('military_wear'), value = 'intendent_wear'})
	elseif grade == 'lieutenant' then
		table.insert(elements, {label = _U('military_wear'), value = 'lieutenant_wear'})
	elseif grade == 'chef' then
		table.insert(elements, {label = _U('military_wear'), value = 'chef_wear'})
	elseif grade == 'boss' then
		table.insert(elements, {label = _U('military_wear'), value = 'boss_wear'})
	elseif grade == 'boss1' then
		table.insert(elements, {label = ('SWAT - Weste'), value = 'swat_wear'})
	end
	...
	if Config.EnableNonFreemodePeds then
		...
		table.insert(elements, {label = 'military wear', value = 'freemode_ped', maleModel = 's_m_y_cop_01', femaleModel = 's_f_y_cop_01'})
...
function OpenVehicleSpawnerMenu(type, station, part, partNum)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle', {
		...
		if data.current.action == 'buy_vehicle' then
			...
			if type == 'car' then
				shopCoords = Config.MilitaryStations[station].Vehicles[partNum].InsideShop
			...
			elseif type == 'helicopter' then
				shopCoords = Config.MilitaryStations[station].Helicopters[partNum].InsideShop
	...
	ESX.TriggerServerCallback('esx_militaryjob:getFineList', function(fines)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine_category', {
			...
			if Config.EnablePlayerManagement then
				TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), 'society_military', _U('fine_total', data.current.fineLabel), data.current.amount)
			else
...
AddEventHandler('esx_phone:cancelMessage', function(dispatchNumber)
	if PlayerData.job and PlayerData.job.name == 'military' and PlayerData.job.name == dispatchNumber then
...
AddEventHandler('esx_militaryjob:hasEnteredEntityZone', function(entity)
	...
	if PlayerData.job and PlayerData.job.name == 'military' and IsPedOnFoot(playerPed) then
...
Citizen.CreateThread(function()
	while true do
		...
		if PlayerData.job and PlayerData.job.name == 'military' then
			...
			for k,v in pairs(Config.MilitaryStations) do
			    ...
			    if Config.EnablePlayerManagement and PlayerData.job.grade_name == 'boss1' then
					for i=1, #v.BossActions, 1 do
						local distance = GetDistanceBetweenCoords(coords, v.BossActions[i], true)
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) and PlayerData.job and PlayerData.job.name == 'military' then
		...
		if IsControlJustReleased(0, 167) and not isDead and PlayerData.job and PlayerData.job.name == 'military' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'military_actions') then
			if Config.MaxInService == -1 then
